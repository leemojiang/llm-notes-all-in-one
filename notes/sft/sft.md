...
SFT 是 Supervised Fine-Tuning，也就是监督微调。它通常接在 Pretrain 之后，用已经整理好的指令、问答或者多轮对话数据，让一个只会“续写文本”的基座模型，进一步学会按照用户输入给出回答。

从训练目标上看，SFT 并没有发明一个全新的 loss。它仍然是 next token prediction：给定前面的 token，预测下一个 token。真正变化的是训练数据的组织方式，以及哪些 token 会参与 loss。
...
这里 \\(p_\theta\\) 表示参数为 \\(\theta\\) 的模型给出的条件概率。这个公式表达的含义是：prompt 只是条件，真正被监督学习的是 assistant 的回答 token。
...
4. [SFT 常见问题](pitfalls.md)：整理 SFT 中容易混淆的点。
