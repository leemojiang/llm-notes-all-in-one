...
根据目标上下文能力,构造数据集
    影响 max_seq_length padding truncation

根据模型参数和数据集规模,构造数据集大小:
    影响 epoch 和 tokens seen

根据实际硬件显存,构造 batch size 和 gradient accumulation steps:
    影响 effective batch size 和 tokens per update

根据loss曲线,调整 learning rate 和 warmup schedule:
    影响训练稳定性和收敛速度
