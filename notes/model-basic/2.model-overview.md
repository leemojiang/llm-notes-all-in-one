...
> MiniMind 里这条主链路的骨架也写得很直接。模型主干 `MiniMind_Dense` 负责 `Embedding`、多层 `MiniMindBlock` 和最后的 `Norm`；`MiniMindForCausalLM` 再在外面包一层 `lm_head`，把 hidden states 投影回词表空间。对应实现来自 [model_minimind.py](/C:/Users/Administrator/Desktop/LLM/llm-from-scratch/src/minimind_learning/model/model_minimind.py:431) 和 [model_minimind.py](/C:/Users/Administrator/Desktop/LLM/llm-from-scratch/src/minimind_learning/model/model_minimind.py:539)。

<details style="color:rgb(128,128,128)">
<summary>MiniMind: 模型主干与 LM head</summary>
...
> MiniMind 里这一层的实现也很直接，就是一个 `nn.Embedding`，然后在 `forward` 里把 `input_ids` 查表成 `[batch_size, seq_len, hidden_size]`。对应实现来自 [model_minimind.py](/C:/Users/Administrator/Desktop/LLM/llm-from-scratch/src/minimind_learning/model/model_minimind.py:443) 和 [model_minimind.py](/C:/Users/Administrator/Desktop/LLM/llm-from-scratch/src/minimind_learning/model/model_minimind.py:507)。

<details style="color:rgb(128,128,128)">
<summary>MiniMind: Embedding 的定义与前向输入输出</summary>
...
> MiniMind 没有使用固定Position Embedding，而是使用了RoPE,先预计算 RoPE 所需的 `freqs_cos` 和 `freqs_sin`，再在 `forward` 里按当前位置切出来传给各层 attention。RoPE请参考RoPE小节的代码实现, 对应实现来自 [model_minimind.py](/C:/Users/Administrator/Desktop/LLM/llm-from-scratch/src/minimind_learning/model/model_minimind.py:450) 和 [model_minimind.py](/C:/Users/Administrator/Desktop/LLM/llm-from-scratch/src/minimind_learning/model/model_minimind.py:510)。

<details style="color:rgb(128,128,128)">
<summary>MiniMind: RoPE 位置向量的预计算与切片</summary>
...
| --- | --- | ---: | ---: | --- |
| GPT-2 | 124M | 768 | 12 | [config](https://huggingface.co/openai-community/gpt2/blob/main/config.json) |
| GPT-2 Medium | 355M | 1024 | 16 | [config](https://huggingface.co/openai-community/gpt2-medium/blob/main/config.json) |
| GPT-2 Large | 774M | 1280 | 20 | [config](https://huggingface.co/openai-community/gpt2-large/blob/main/config.json) |
| GPT-2 XL | 1.5B | 1600 | 25 | [config](https://huggingface.co/openai-community/gpt2-xl/blob/main/config.json) |
| Llama-2-7B | 7B | 4096 | 32 | [config](https://huggingface.co/NousResearch/Llama-2-7b-hf/blob/main/config.json) |
| Llama-3-8B | 8B | 4096 | 32 | [config](https://huggingface.co/NousResearch/Meta-Llama-3-8B/blob/main/config.json) |
| Qwen2.5-7B | 7B | 3584 | 28 | [config](https://huggingface.co/Qwen/Qwen2.5-7B/blob/main/config.json) |
...
1. 先定模型总规模
   比如是 20M、100M，还是 1B 以上。这个规模先把 hidden size 的上限大致卡住。

2. 再定层数和头数
   同样的总参数量，可以做成更宽更浅，也可以做成更窄更深，所以 hidden size 不能脱离层数单独看。

3. 最后让 \\( d_{\text{model}} \\) 和 \\( n_{\text{heads}} \\) 对齐
   也就是让它能自然整除，这样 \\( d_{\text{head}} \\) 会比较规整，实现也更自然。
...
> MiniMind 这里也直接使用了 `weight tying`，也就是让输入 embedding 的权重和 `lm_head` 共享。对应实现来自 [model_minimind.py](/C:/Users/Administrator/Desktop/LLM/llm-from-scratch/src/minimind_learning/model/model_minimind.py:545)。

<details style="color:rgb(128,128,128)">
<summary>MiniMind: 输入 embedding 和 LM head 共享权重</summary>
...
| 模型 | 位置方案 | 相关配置 |
| --- | --- | --- |
| GPT-2 | learned absolute position embedding | `n_positions = 1024`，[config](https://huggingface.co/openai-community/gpt2/blob/main/config.json) |
| Llama-3-8B | RoPE | `max_position_embeddings = 8192`, `rope_theta = 500000.0`，[config](https://huggingface.co/NousResearch/Meta-Llama-3-8B/blob/main/config.json) |
| Qwen2.5-7B | RoPE | `max_position_embeddings = 131072`, `rope_theta = 1000000.0`，[config](https://huggingface.co/Qwen/Qwen2.5-7B/blob/main/config.json) |
...
不过 RoPE 本身已经不只是“位置编码层的一个小变体”了，它和 attention 的计算过程紧密绑定，所以我把它单独拆成了一节：[RoPE: 位置编码如何进入 Attention?](../model-extra/2a.rope.md)
...
> MiniMind 里 Norm 的实现和这里的讨论是直接对应的。它既保留了 `RMSNorm`，也额外实现了一个 `LayerNorm` 版本，两者都清楚标了 `[batch_size, seq_len, dim]` 这类 shape。对应实现来自 [model_minimind.py](/C:/Users/Administrator/Desktop/LLM/llm-from-scratch/src/minimind_learning/model/model_minimind.py:13)。

<details style="color:rgb(128,128,128)">
<summary>MiniMind: RMSNorm 和 LayerNorm 的实现</summary>
...
如果严格对照前面那张列表，它**没有再单独定义一个额外的 Output Dropout**；输出侧主要是直接接 `lm_head` 做词表投影。

对应实现都在 [model_minimind.py](/C:/Users/Administrator/Desktop/LLM/llm-from-scratch/src/minimind_learning/model/model_minimind.py) 里：

- `MiniMind_Dense.dropout`：embedding 后的 dropout，见 [model_minimind.py](/C:/Users/Administrator/Desktop/LLM/llm-from-scratch/src/minimind_learning/model/model_minimind.py:448)
- `Attention.attn_dropout`：attention 权重上的 dropout，见 [model_minimind.py](/C:/Users/Administrator/Desktop/LLM/llm-from-scratch/src/minimind_learning/model/model_minimind.py:141)
- `Attention.resid_dropout`：`o_proj` 之后、回到残差支路之前的 dropout，见 [model_minimind.py](/C:/Users/Administrator/Desktop/LLM/llm-from-scratch/src/minimind_learning/model/model_minimind.py:142)
- `FeedForward.dropout`：FFN 内部、`down_proj` 输出后的 dropout，见 [model_minimind.py](/C:/Users/Administrator/Desktop/LLM/llm-from-scratch/src/minimind_learning/model/model_minimind.py:88)

<details style="color:rgb(128,128,128)">
<summary>MiniMind: Dropout 的几个实际位置</summary>
...
| --- | ---: | ---: | ---: | --- | --- |
| Llama-2-7B | 4096 | 11008 | 2.69 | SiLU / SwiGLU 系 | [config](https://huggingface.co/NousResearch/Llama-2-7b-hf/blob/main/config.json) |
| Llama-3-8B | 4096 | 14336 | 3.5 | SiLU | [config](https://huggingface.co/NousResearch/Meta-Llama-3-8B/blob/main/config.json) |
| Qwen2.5-7B | 3584 | 18944 | 5.29 | SiLU | [config](https://huggingface.co/Qwen/Qwen2.5-7B/blob/main/config.json) |
...
MiniMind 这里的 FFN 不是最朴素的两层 MLP，而是更接近 LLaMA 系列的 gated FFN：`gate_proj` 和 `up_proj` 做逐元素乘法，再经过 `down_proj` 投回 hidden size。激活函数和 Dropout 也都在这里落地了。对应实现来自 [model_minimind.py](/C:/Users/Administrator/Desktop/LLM/llm-from-scratch/src/minimind_learning/model/model_minimind.py:59)。

<details style="color:rgb(128,128,128)">
<summary>MiniMind: FeedForward / gated FFN 的实现</summary>
...
1. 先定 hidden size
2. 再选一个能整除 hidden size 的 head 数
3. 看最后得到的 head dim 是否落在主流实现里常见的范围内
...
> 第一段是 `MiniMindBlock`：它把 `input_layernorm -> self_attn -> residual -> post_attention_layernorm -> mlp` 这条主路径写得很清楚。对应实现来自 [model_minimind.py](/C:/Users/Administrator/Desktop/LLM/llm-from-scratch/src/minimind_learning/model/model_minimind.py:378)。

<details style="color:rgb(128,128,128)">
<summary>MiniMind: 一个 Transformer block 的主路径</summary>
...
> 第二段是 `Attention.forward` 里和本节主线最直接对应的部分。这里只保留了 `Q/K/V` 投影、RoPE 注入、softmax 权重和输出重排这些和正文公式一一对应的部分；MiniMind 里实际还包含 `KV cache`、`repeat_kv` 这些更偏后续专题的实现，这里先不展开。对应实现来自 [model_minimind.py](/C:/Users/Administrator/Desktop/LLM/llm-from-scratch/src/minimind_learning/model/model_minimind.py:151)。

<details style="color:rgb(128,128,128)">
<summary>MiniMind: Attention 核心计算路径</summary>
...
所以我觉得，这部分内容更适合**按问题去读，而不是按顺序去读**。比较自然的方式是先想清楚：自己现在更关心的到底是哪一类问题，比如是显存、推理速度，还是长上下文能力；然后再去挑对应的小节来看。

后面我也准备把更多相关的方法逐步补充到这一组专题里。比如：

- 有的在改位置信息进入 attention 的方式，比如 [RoPE: 位置编码如何进入 Attention?](../model-extra/2a.rope.md)
- 有的在改 attention 的底层实现方式，比如 [FlashAttention: attention 为什么还能更快?](../model-extra/2b.flash-attention.md)
- 有的在改多头结构本身，比如 [GQA: 为什么 Query head 和 KV head 可以不一样?](../model-extra/2c.gqa.md)
- 有的在改推理阶段的缓存方式，比如 [KV Cache: 自回归推理为什么能避免重复计算?](../model-extra/2d.kv-cache.md)
- 有的在改长上下文扩展能力，比如 [Long Context: 长上下文能力通常在改什么?](../model-extra/2e.long-context.md)
