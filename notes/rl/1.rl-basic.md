...
Robbins-Monro 算法属于 stochastic approximation。它解决的是这样的问题：我们想求某个方程 \\(g(w)=0\\) 的根，但拿不到精确的 \\(g(w)\\)，只能拿到带噪声的观测。
...
它和 RL 的关系在于：TD learning 可以被理解为一种 stochastic approximation，用采样到的 TD target 去逼近 Bellman equation 的解。
...
Monte Carlo 使用完整 return，不依赖当前 value estimate，因此是 non-bootstrapping。
