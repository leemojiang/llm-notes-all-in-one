...
但更新的不再是某一个表格 entry，而是参数 \\(w\\)。通常可以把 TD error 当作监督信号，用梯度下降更新 \\(w\\)。

它仍然是 on-policy，因为下一步动作 \\(a_{t+1}\\) 来自当前策略。

#### Q: On-policy 和 off-policy 的区别是什么？

更具体地说，在 Sarsa with function approximation 中，behavior policy 和 target policy 是同一个策略。智能体用当前策略 \\(\pi\\) 采样动作：
...
然后用这个真实采样到的 \\(a_{t+1}\\) 构造 TD target：
...
所以 Sarsa 的逻辑是：我按照当前策略行动，也按照当前策略的实际行动结果来学习当前策略的价值。Function approximation 只是把表格 \\(q(s,a)\\) 换成了 \\(\hat{q}(s,a,w)\\)，并不改变它 on-policy 的本质。
...
这个方法是 DQN 的前身。它的核心风险是：Q-learning 本来就是 off-policy，再叠加 function approximation 和 bootstrapping，训练可能变得不稳定。

为什么说它是 off-policy？关键在于：产生样本的 behavior policy 和被学习的 target policy 不同。

在交互时，动作 \\(a_t\\) 可以来自一个带探索的 behavior policy \\(\beta\\)，例如 ε-greedy：

<script type="math/tex; mode=display">
a_t\sim \beta(\cdot\mid s_t)
</script>

但是 Q-learning 的 TD target 并不使用行为策略下一步真实会采样到的动作，而是假设下一步选择 greedy action：
...
这个 greedy action 对应的是 target policy：
...
所以 Q-learning 的逻辑是：我可以用一个带探索的策略去采样数据，但我更新的是 greedy target policy 的价值。Function approximation 同样只改变 value 的表示方式，不改变 Q-learning off-policy 的本质。
...
它的基本目标仍然是最小化 TD target 和当前 Q 估计之间的差异：
