...
# 代码和 train_pretrain.py 基本一致，主要区别是 Dataset、batch_size、学习率和默认加载的权重。
...
if __name__ == "__main__":
    ...
    parser.add_argument("--ckpt_dir", type=str, default="../checkpoints", help="checkpoint保存目录")
    ...
    # ========== 8. 开始训练 ==========
    Logger("***** 开始训练 *****")
    Logger(f"模型参数量: {sum(p.numel() for p in model.parameters()) / 1e6:.2f}M")
    Logger(f"Device: {args.device} Batch size: {args.batch_size} Accumulation steps: {args.accumulation_steps}")
